...
const ToolNom = require('../models/tool_nom')
...
async function getTools(req, res) {
  try {
    ...
    const tools = rows.map((tool) => {
      const property = {} // Пример формата property, здесь нужна дополнительная логика для заполнения этого объекта
      // Здесь должен быть код, который заполняет объект property
      // на основе данных из tool.property или других источников

      return {
        id: tool.id,
        name: tool.name,
        property: property, // Полагаем, что tool.property - это JSON объект
        sklad: tool.sklad,
        norma: tool.norma,
        limit: tool.limit,
      }
